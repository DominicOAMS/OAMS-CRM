...
(function () {
  ...
  let dupState = { sheet: null, key: null, groups: [] };

  document.getElementById('findDuplicatesBtn').addEventListener('click', () => {
    const view = window.currentView;
    const cols = (window.currentTableData.columns || []).filter(c => !isIdColumn(c.name)).map(c => c.name);
    if (cols.length === 0) { Swal.fire({ title: 'Nothing to check', text: 'This tab has no columns to match on.', icon: 'info', heightAuto: false, scrollbarPadding: false }); return; }
    const preferred = { Accounts: 'Account Name', Leads: 'Email', Contacts: 'Email' }[view];
    const opts = cols.map(c => `<option value="${escapeHtml(c)}" ${c === preferred ? 'selected' : ''}>${escapeHtml(c)}</option>`).join('');
    Swal.fire({
      title: 'Find Duplicates',
      html: `<div class="form-field"><label class="form-label">Match records by</label><select id="dup-key" class="swal2-select swal-field-select" style="width:100%;">${opts}</select></div>`,
      showCancelButton: true,
      confirmButtonText: 'Find',
      confirmButtonColor: '#0088ff',
      heightAuto: false,
      scrollbarPadding: false,
      preConfirm: () => document.getElementById('dup-key').value
    }).then(r => {
      if (!r.isConfirmed) return;
      const key = r.value;
      Swal.fire({ title: 'Scanning...', allowOutsideClick: false, heightAuto: false, scrollbarPadding: false, didOpen: () => Swal.showLoading() });
      google.script.run
        .withSuccessHandler(res => { Swal.close(); dupState = { sheet: view, key: key, groups: res.groups }; showDuplicatesModal(); })
        .withFailureHandler(err => Swal.fire('Error', err.message, 'error'))
        .findDuplicateGroups(view, key);
    });
  });
  ...
  function showDuplicatesModal() {
    ...
    if (!groups || groups.length === 0) {
      Swal.fire({ title: 'No duplicates found', text: `No records share the same ${dupState.key}.`, icon: 'success', heightAuto: false, scrollbarPadding: false });
      return;
    }
    let html = '<div style="text-align:left; max-height:400px; overflow-y:auto;">';
    groups.forEach((g, gi) => {
      html += `<div class="dup-group"><div class="dup-group-head">${escapeHtml(g.key)} <span class="home-count">${g.members.length}</span></div>`;
      g.members.forEach((m, mi) => {
        html += `<label class="dup-member"><input type="radio" name="dup-master-${gi}" value="${m.rowIndex}" ${mi === 0 ? 'checked' : ''}><span class="dup-member-label">${escapeHtml(m.label)}</span><span class="dup-filled">${m.filledCount} filled</span></label>`;
      });
      html += `<button class="btn btn-secondary btn-danger-outline" style="margin-top:8px; font-size:12px; padding:6px 12px;" onclick="doMergeGroup(${gi})">Merge this group</button></div>`;
    });
    html += '</div>';
    Swal.fire({ title: 'Duplicates', html: html, showConfirmButton: false, showCloseButton: true, width: '560px', heightAuto: false, scrollbarPadding: false });
  }

  window.doMergeGroup = function(gi) {
    const g = dupState.groups[gi];
    if (!g) return;
    const sel = document.querySelector(`input[name="dup-master-${gi}"]:checked`);
    if (!sel) return;
    const masterRowIndex = parseInt(sel.value, 10);
    const rowIndices = g.members.map(m => m.rowIndex);
    const masterLabel = (g.members.find(m => m.rowIndex === masterRowIndex) || {}).label || '';

    Swal.fire({
      title: 'Merge duplicates?',
      html: `Keep <b>${escapeHtml(masterLabel)}</b> and merge &amp; delete ${rowIndices.length - 1} other record(s). Blanks in the kept record are filled from them and links are re-pointed. <b>This can't be undone.</b>`,
      icon: 'warning',
      showCancelButton: true,
      confirmButtonColor: '#d93025',
      ...
      if (!r.isConfirmed) return;
      Swal.fire({ title: 'Merging...', allowOutsideClick: false, heightAuto: false, scrollbarPadding: false, didOpen: () => Swal.showLoading() });
      google.script.run
        .withSuccessHandler(data => {
          renderTable(data); // table behind the modal reflects the merge
          // Re-scan (row indices shifted) and rebuild the list from fresh groups.
          google.script.run
            .withSuccessHandler(res => { Swal.close(); dupState.groups = res.groups; showDuplicatesModal(); })
            .withFailureHandler(() => Swal.close())
            .findDuplicateGroups(dupState.sheet, dupState.key);
        })
        .withFailureHandler(err => Swal.fire('Error', err.message, 'error'))
        .mergeRecords(dupState.sheet, rowIndices, masterRowIndex);
    });
  };
